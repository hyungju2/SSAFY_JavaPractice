...
public class EmpUI implements ActionListener {
	...
	private void updateRecord() {
		...
		String dept = txtDept.getText().trim();
		if (empNo.equals("") || name.equals("") || position.equals("") || dept.equals("")) {
			lblStatus.setText("직원 정보를 모두 입력해 주세요.");
			return;
		}
		try {
			mgr.update(new Employee(Integer.parseInt(empNo), name, position, dept));
			lblStatus.setText("등록 성공");
			clearField();
			showList();
		} catch (Exception e) { //////////////// DulicateException 이 구현 되면 주석 제거
			lblStatus.setText(e.toString()); //////////////// DulicateException 이 구현 되면 주석 제거
		}
	...
	private void searchRecord() {
		...
		try {
			...
			lblStatus.setText("RecordNotFoundException");
		}
		list.removeAll();
	...
	private void deleteRecord() {
		String empNo = txtEmpNo.getText().trim();
		if (empNo.equals("")) {
			lblStatus.setText("직원번호를 먼저 입력하세요.");
			return;
		}
		try {
